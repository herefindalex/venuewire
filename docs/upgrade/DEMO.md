...
```bash
./bin/venuewire --venue deribit public-stream \
...
```bash
./bin/venuewire --venue deribit instrument --name BTC-PERPETUAL
./bin/venuewire --venue deribit ticker --instrument BTC-PERPETUAL
```
...
```bash
./bin/venuewire --venue deribit order status --order-id <native-id>
./bin/venuewire --venue deribit order trades --order-id <native-id>
