...
describe('browser API boundary', () => {
  ...
  it('uses the protected refresh endpoint instead of polling per browser request', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({ account: {} }), { status: 200, headers: { 'Content-Type': 'application/json' } }));
    await api.refreshAccount('bybit');
    const [path, init] = fetchMock.mock.calls[0];
    expect(path).toBe('/api/venues/bybit/account/refresh');
